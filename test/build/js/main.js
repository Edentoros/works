...
var checks = [];
var input = document.querySelectorAll('input[name="js"]');
var arrow = document.querySelector('.form__diagram-arrow');

for (var i = 0; i < input.length; i++) {
  input[i].addEventListener('change', getChecked);
}
...
function getChecked() {
  ...
  for (var _i = 0; _i < checks.length; _i++) {
    if (checks.length > 0) {
      arrow.className = 'form__diagram-arrow yellow';
      $('.form__level-number').animate({
        num: 157
      }, {
        duration: 600,
        step: function step(num) {
          this.innerHTML = Math.trunc(num);
        }
      });
    }

    if (checks.length > 1) {
      arrow.className = 'form__diagram-arrow green';
      $('.form__level-number').animate({
        num: 420
      }, {
        duration: 150,
    ...
    if (checks.length > 2) {
      arrow.className = 'form__diagram-arrow blue';
      $('.form__level-number').animate({
        num: 954
      }, {
        duration: 10,
